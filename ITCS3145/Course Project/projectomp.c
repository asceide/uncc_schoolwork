...
To compile the code, we use
        gcc -g -Wall -fopenmp -o projectomp stb_image/stb_image.h stb_image/stb_image_write.h projectomp.c -lm

To run the code, type
        ./projectomp ${input color image} ${output grayscale image} ${image type} ${number of threads }
        ...
        For example,
        ./projectomp lena1.png lena2.png 1 4
        ./projectomp lizard1.jpg lizard2.jpg 2 4
*/
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
...
void colorToGrayscale(unsigned char *gray_img, unsigned char * color_img, int width, int height, int t_count);
...
void colorToGrayscale(unsigned char *gray_img, unsigned char * color_img, int width, int height, int t_count){
    ...
    int row;
    int col;
#   pragma omp parallel for num_threads(t_count) collapse(2) \
    default(none) private(row, col, pixel) \
    shared(gray_img, color_img, width, height)
